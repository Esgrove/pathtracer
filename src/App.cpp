...
bool App::handleEvent(const Window::Event& ev)
{
	...
	case Action_ChangeLight:
		...
		m_light_index += 1;
		if (m_light_index >= m_areaLights->size()) { m_light_index = 0; }
	...
	case Action_PathTraceMode:
		...
		if (m_RTMode)
		{
			...
			m_pathtrace_renderer->startPathTracingProcess(m_mesh.get(), m_areaLights.get(), m_rt.get(), &m_img, m_useRussianRoulette ? -m_numBounces : m_numBounces, m_cameraCtrl);
			::printf("\nRendering started:\n");
			::printf("Pass 0\n");
		}
		else
...
void App::renderFrame(GLContext* gl)
{
	...
	if (worldToClip != previous_camera || clear_on_next_frame)
	{
		...
		m_pathtrace_renderer->setAAmode(m_sobol_block);
		m_pathtrace_renderer->setTermination(m_termination); 
		m_pathtrace_renderer->startPathTracingProcess(m_mesh.get(), m_areaLights.get(), m_rt.get(), &m_img, 
													  m_useRussianRoulette ? -m_numBounces : m_numBounces, m_cameraCtrl);
	}
