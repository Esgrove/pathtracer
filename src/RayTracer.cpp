...
Mat3f formBasis(const Vec3f& n) {
	...
	int minV = 0;
	if (abs(q.y) < abs(q[minV])) { minV = 1; }
	if (abs(q.z) < abs(q[minV])) { minV = 2; }
	q[minV] = 1.0f;
...
void RayTracer::loadHierarchy(const char* filename, std::vector<RTTriangle>& triangles)
{
	...
	// index list
	std::vector<unsigned int> list; unsigned __int32 temp;
...
RaycastResult RayTracer::raycast(const Vec3f& orig, const Vec3f& dir, bool occlusion) const
{
	...
	// currentNodeIndex holds the offset (index) of the node to be visited. It starts with a value of 0, representing the root of the tree.
	// The nodes that still need to be visited are stored in the nodesToVisit[] array, which acts as a stack.
	// toVisitOffset holds the offset (index) to the next free element in the stack.
